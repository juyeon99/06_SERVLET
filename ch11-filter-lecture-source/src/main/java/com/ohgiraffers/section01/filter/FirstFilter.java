...
* */
@WebFilter("/first/*")  // /first로 시작하는 모든 url 요청에 필터를 적용
public class FirstFilter implements Filter {
    public FirstFilter(){   // def-constructor
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        // Filter 인스턴스가 최초 생성될 때 호출
        System.out.println("Filter init 호출");
    }

    @Override
    public void destroy() {
        // Filter 인스턴스가 소멸될 때 호출 (= 톰캣 종료시)
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        System.out.println("Filter doFilter() 호출"); // 처리하러 갈 때

        // 필터에서 처리할 코드가 작성되는 공간

        // 처리 이후 다음 필터 혹은 서블릿의 doGet/doPost를 호출
        filterChain.doFilter(servletRequest,servletResponse);

        // 서블릿에서 처리 이후에 다시 수행할 내용이 있으면 작성
        System.out.println("Servlet 요청 수행 완료"); // 처리 후 올 때
    }
}
